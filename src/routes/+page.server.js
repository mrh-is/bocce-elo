...
export async function load() {
	...
	try {
		officialRankings = await getOfficialRankings(
			PUBLIC_SHEET_ID, PUBLIC_GOOGLE_API_KEY, SUMMARY_TAB, RANKINGS_NAME_COL, RANKINGS_RANK_COL
		);
	} catch (err) {
	...
	const resolveMatchups = (rows) => parseMatchupsWithCourts(rows).map((m) => {
		...
		return { teamA: tA, teamB: tB, court: m.court, probA: Math.round(expectedScore(rA, rB) * 100) };
	});

	let upcomingMatches = [];
	if (UPCOMING_TAB) {
		try {
			const rows = await fetchTab(PUBLIC_SHEET_ID, PUBLIC_GOOGLE_API_KEY, UPCOMING_TAB);
			upcomingMatches = resolveMatchups(rows);
		} catch (err) {
			console.warn(`[load] Could not fetch upcoming tab "${UPCOMING_TAB}": ${err.message}`);
		}
	} else {
	...
	for (const m of upcomingMatches) {
		...
		addUpcoming(m.teamB, { opponent: m.teamA, prob: 100 - m.probA, court: m.court });
	}

	const leaderboard = Object.keys(ratings)
		.sort((a, b) => ratings[b] - ratings[a])
		.map((name, i) => {
			const rec = records[name] ?? { wins: 0, losses: 0, ties: 0, pointsFor: 0, pointsAgainst: 0 };
			const eloRank = i + 1;
