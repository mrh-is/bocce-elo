<script>
	...
	const hasUpcoming = $derived(data.leaderboard.some((t) => t.upcoming.length > 0));
</script>
...
<div class="layout">
	<header>
		...
		<p class="season-label">ELO Power Rankings · {data.seasonLabel} · Pittsburgh, PA</p>
		<div class="deco">◆ ◆ ◆</div>
	</header>
		...
		<table>
			...
			<tbody>
								...
								{#if team.upcoming.length > 0}
									{#each team.upcoming as game}
										<div class="game-line">
											...
											<span class:odds-fav={game.prob >= 50} class:odds-dog={game.prob < 50}
												>{game.prob}%</span
											>
										</div>
									{/each}
				...
				{#if filtered.length === 0}
					<tr><td colspan={hasUpcoming ? 8 : 7} class="empty">No teams match "{search}" 🤷</td></tr>
				{/if}
			</tbody>
		</table>
	</div>
	...
	<footer>
		<p>
			...
			<span class="elo-worse">Red ELO #</span> = ELO ranks lower ·
			ELO accounts for score margins
		</p>
		<p>
			Updated {new Date(data.lastUpdated).toLocaleString()} ·
			<a href={data.sheetUrl} target="_blank" rel="noopener noreferrer">Source Spreadsheet ↗</a>
		</p>
	</footer>
</div>
